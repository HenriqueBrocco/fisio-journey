...
export default function PassadaLateral({
        ...
        async function carregarConfig() {
            ...
            try {
                const data = await apiFetch<AssignmentConfigResponse[]>(
                    `/v1/exercise-configs?patient_user_id=${patientUserId}&exercise_id=${exerciseId}`
                );
                ...
                setConfigClinica((prev) => ({
                    ...prev,
                    seriesTotais: cfg.num_series ?? prev.seriesTotais,
                    repsEsquerda: cfg.num_reps ?? prev.repsEsquerda,
                    repsDireita: cfg.num_reps ?? prev.repsDireita,
                    descansoRep: cfg.descanso_rep ?? prev.descansoRep,
                    descansoSerie: cfg.descanso_serie ?? prev.descansoSerie,
                    metaAbertura: cfg.meta_extensao ?? prev.metaAbertura,
    ...
    async function handleFinalizarSessaoEVoltar() {
        try {
            setFinalizandoSessao(true);

            const totalReps = contagemRef.current.repsEsq + contagemRef.current.repsDir;
            const totalCorretas = relatorioFinal.reduce((acc, item) => acc + (item.corretas || 0), 0);
            const totalIncorretas = relatorioFinal.reduce((acc, item) => acc + (item.incorretas || 0), 0);
            const totalTentativas = totalCorretas + totalIncorretas;
            const accuracy = totalTentativas > 0 ? Math.round((totalCorretas / totalTentativas) * 100) : 0;

            const totalCompensacoes = relatorioFinal.reduce(
                (acc, item) => acc + (item.compensacoes || 0),
                0
            );

            const alerts =
                totalCompensacoes > 0
                    ? [`Compensações de postura detectadas: ${totalCompensacoes}`]
                    : [];

            const response = await apiFetch<FinalizeSessionResponse>(`/v1/sessions/${sessionId}/finalize`, {
                method: "POST",
            ...
            if (response.new_achievements && response.new_achievements.length > 0) {
                setUnlockedAchievements(response.new_achievements);
            ...
            alert("Não foi possível finalizar a sessão.");
        } finally {
                    ...
                    if (resultados.landmarks && resultados.landmarks[0] && resultados.worldLandmarks && resultados.worldLandmarks[0]) {
                        ...
                        if (menuAbertoRef.current) {
                            if (maoEsqLevantada) {
                                ...
                                if (contadorGestoEsqRef.current >= 10) { setMenuAberto(false); contadorGestoEsqRef.current = 0; setProgressoEsq(0); }
                            } else { contadorGestoEsqRef.current = 0; setProgressoEsq(0); }

                            if (maoDirLevantada) {
                                contadorGestoDirRef.current += 1; setProgressoDir((contadorGestoDirRef.current / 10) * 100);
                                if (contadorGestoDirRef.current >= 10) { stopCameraAndTracking(); navigate("/dashboard"); }
                            } else { contadorGestoDirRef.current = 0; setProgressoDir(0); }
                        }
                        else if (refEmDescanso.current) {
                            if (maoEsqLevantada) {
                                contadorGestoEsqRef.current += 1; setProgressoEsq((contadorGestoEsqRef.current / 20) * 100);
            ...
            {relatorioFinal.length > 0 && (
                <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0, 0, 0, 0.85)', zIndex: 60, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', backdropFilter: 'blur(8px)' }}>
                    <div style={{ backgroundColor: '#67807eb9', padding: '40px', borderRadius: '20px', border: '4px solid #67B5A2', width: '80%', maxWidth: '800px', display: 'flex', flexDirection: 'column', alignItems: 'center', boxShadow: '0 20px 50px #444E4D' }}>
                        ...
                        <button
                            onClick={handleFinalizarSessaoEVoltar}
                            disabled={finalizandoSessao}
                            style={{
                                padding: '20px 50px',
                                fontSize: '1.5rem',
                                backgroundColor: '#67B5A2',
                                color: 'white',
                                border: 'none',
                                borderRadius: '10px',
                                cursor: finalizandoSessao ? 'not-allowed' : 'pointer',
                                fontWeight: 'bold',
                                boxShadow: '0 10px 20px #444E4D',
                                opacity: finalizandoSessao ? 0.7 : 1,
                            }}
                        >
                            {finalizandoSessao ? "Finalizando..." : "Voltar ao Menu Inicial"}
                        </button>
                    </div>
                </div>
            )}
            ...
            {showAchievementModal && unlockedAchievements.length > 0 && (
                ...
                >
                    ...
                    >
                        ...
                        <h2 style={{ margin: 0, fontSize: "2rem", color: "#111827" }}>
                            Nova conquista desbloqueada!
                        </h2>
                        <p style={{ marginTop: "8px", color: "#6b7280", fontSize: "1rem" }}>
                            Você ganhou uma nova conquista durante esta sessão.
                        </p>
                                ...
                                >
                                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
                                        <div>
                                            ...
                                            <div style={{ marginTop: "4px", color: "#6b7280", fontSize: "0.95rem" }}>
                                                {ach.description}
                                            </div>
                                        </div>
